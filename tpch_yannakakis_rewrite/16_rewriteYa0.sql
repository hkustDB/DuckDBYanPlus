create or replace view semiUp6202188780695040087 as select p_partkey as v6, p_brand as v9, p_type as v10, p_size as v11 from part AS part where (p_partkey) in (select ps_partkey from partsupp AS partsupp WHERE (ps_suppkey NOT IN (SELECT s_suppkey FROM supplier WHERE s_comment LIKE '%Customer%Complaints%'))) and p_brand<> 'Brand#45' and p_type NOT LIKE 'MEDIUM POLISHED%' and p_size IN (49,14,23,45,19,3,36,9);
...
create or replace view semiDown7916621178234031947 as select ps_partkey as v6 from partsupp AS partsupp WHERE (ps_suppkey NOT IN (SELECT s_suppkey FROM supplier WHERE s_comment LIKE '%Customer%Complaints%')) AND (ps_partkey) in (select v6 from semiDown3357453452641617529);
...
select v9, v10, v11, CAST(annot AS BIGINT) AS v15 from aggView7794489038438901440;
