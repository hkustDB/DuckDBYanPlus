...
CREATE OR REPLACE TEMP VIEW ya_18b_rewriteYa23_r3_base_ci AS
...
CREATE OR REPLACE TEMP VIEW ya_18b_rewriteYa23_r3_base_it1 AS
SELECT it1.id AS it1__id,
       CAST(COUNT(*) AS HUGEINT) AS annot
FROM ya_18b_rewriteYa23_down_it1 AS it1
GROUP BY it1.id;

CREATE OR REPLACE TEMP VIEW ya_18b_rewriteYa23_r3_base_it2 AS
SELECT it2.id AS it2__id,
       CAST(COUNT(*) AS HUGEINT) AS annot
FROM ya_18b_rewriteYa23_down_it2 AS it2
GROUP BY it2.id;

CREATE OR REPLACE TEMP VIEW ya_18b_rewriteYa23_r3_base_mi AS
SELECT mi.movie_id AS mi__movie_id,
       mi.info_type_id AS mi__info_type_id,
       mi.info AS mi__info,
       CAST(COUNT(*) AS HUGEINT) AS annot
FROM ya_18b_rewriteYa23_down_mi AS mi
GROUP BY mi.movie_id, mi.info_type_id, mi.info;

CREATE OR REPLACE TEMP VIEW ya_18b_rewriteYa23_r3_base_mi_idx AS
SELECT mi_idx.movie_id AS mi_idx__movie_id,
       mi_idx.info_type_id AS mi_idx__info_type_id,
       mi_idx.info AS mi_idx__info,
       CAST(COUNT(*) AS HUGEINT) AS annot
FROM ya_18b_rewriteYa23_down_mi_idx AS mi_idx
GROUP BY mi_idx.movie_id, mi_idx.info_type_id, mi_idx.info;

CREATE OR REPLACE TEMP VIEW ya_18b_rewriteYa23_r3_base_n AS
SELECT n.id AS n__id,
       CAST(COUNT(*) AS HUGEINT) AS annot
FROM ya_18b_rewriteYa23_down_n AS n
GROUP BY n.id;

CREATE OR REPLACE TEMP VIEW ya_18b_rewriteYa23_r3_base_t AS
SELECT t.id AS t__id,
       t.title AS t__title,
       CAST(COUNT(*) AS HUGEINT) AS annot
FROM ya_18b_rewriteYa23_down_t AS t
GROUP BY t.id, t.title;

CREATE OR REPLACE TEMP VIEW ya_18b_rewriteYa23_r3_join_1 AS
...
FROM ya_18b_rewriteYa23_r3_base_mi AS round3_left
JOIN ya_18b_rewriteYa23_r3_base_it1 AS round3_right
  ON (round3_right.it1__id = round3_left.mi__info_type_id)
GROUP BY round3_left.mi__info, round3_left.mi__movie_id;

CREATE OR REPLACE TEMP VIEW ya_18b_rewriteYa23_r3_join_2 AS
SELECT round3_right.mi__info AS mi__info,
       ...
       SUM(round3_left.annot * round3_right.annot) AS annot
FROM ya_18b_rewriteYa23_r3_base_mi_idx AS round3_left
JOIN ya_18b_rewriteYa23_r3_join_1 AS round3_right
  ON (round3_right.mi__movie_id = round3_left.mi_idx__movie_id)
GROUP BY round3_right.mi__info, round3_left.mi_idx__info, round3_right.mi__movie_id, round3_left.mi_idx__movie_id, round3_left.mi_idx__info_type_id;

CREATE OR REPLACE TEMP VIEW ya_18b_rewriteYa23_r3_join_3 AS
SELECT round3_left.mi__info AS mi__info,
       round3_left.mi_idx__info AS mi_idx__info,
       ...
       round3_left.mi_idx__info_type_id AS mi_idx__info_type_id,
       SUM(round3_left.annot * round3_right.annot) AS annot
FROM ya_18b_rewriteYa23_r3_join_2 AS round3_left
JOIN ya_18b_rewriteYa23_r3_base_t AS round3_right
  ON (round3_right.t__id = round3_left.mi__movie_id)
 AND (round3_right.t__id = round3_left.mi_idx__movie_id)
GROUP BY round3_left.mi__info, round3_left.mi_idx__info, round3_right.t__title, round3_right.t__id, round3_left.mi__movie_id, round3_left.mi_idx__movie_id, round3_left.mi_idx__info_type_id;

CREATE OR REPLACE TEMP VIEW ya_18b_rewriteYa23_r3_join_4 AS
SELECT round3_left.ci__movie_id AS ci__movie_id,
       SUM(round3_left.annot * round3_right.annot) AS annot
FROM ya_18b_rewriteYa23_r3_base_ci AS round3_left
JOIN ya_18b_rewriteYa23_r3_base_n AS round3_right
  ON (round3_right.n__id = round3_left.ci__person_id)
GROUP BY round3_left.ci__movie_id;

CREATE OR REPLACE TEMP VIEW ya_18b_rewriteYa23_r3_join_5 AS
SELECT round3_left.mi__info AS mi__info,
       round3_left.mi_idx__info AS mi_idx__info,
       round3_left.t__title AS t__title,
       round3_left.mi_idx__info_type_id AS mi_idx__info_type_id,
       SUM(round3_left.annot * round3_right.annot) AS annot
FROM ya_18b_rewriteYa23_r3_join_3 AS round3_left
JOIN ya_18b_rewriteYa23_r3_join_4 AS round3_right
...
GROUP BY round3_left.mi__info, round3_left.mi_idx__info, round3_left.t__title, round3_left.mi_idx__info_type_id;

CREATE OR REPLACE TEMP VIEW ya_18b_rewriteYa23_r3_join_6 AS
SELECT round3_right.mi__info AS mi__info,
       round3_right.mi_idx__info AS mi_idx__info,
       round3_right.t__title AS t__title,
       SUM(round3_left.annot * round3_right.annot) AS annot
FROM ya_18b_rewriteYa23_r3_base_it2 AS round3_left
JOIN ya_18b_rewriteYa23_r3_join_5 AS round3_right
...
FROM ya_18b_rewriteYa23_r3_join_6 AS round3_result;
