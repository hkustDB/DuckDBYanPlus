...
PhysicalCreateBF *PhysicalPlanGenerator::CreatePlanFromRelated(LogicalCreateBF &op) {
	if (!op.physical) {
		...
		auto filter_type = Settings::Get<YanplusSemijoinFilterSetting>(context);
		auto &create_bf =
		    Make<PhysicalCreateBF>(plan.types, op.filter_plans, op.min_max_to_create, op.min_max_applied_cols,
		                           op.estimated_cardinality, op.can_stop, filter_type);
