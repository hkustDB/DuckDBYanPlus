create or replace view semiUp3238806025272546318 as select CommentId as v3, ParentMessageId as v1 from Comment_replyOf_Message_T AS Comment_replyOf_Message_T where (CommentId) in (select (CommentId) from Comment_hasTag_Tag AS cht2);
create or replace view semiUp6900398025808263048 as select MessageId as v1, TagId as v2 from Message_hasTag_Tag_T AS Message_hasTag_Tag_T where (MessageId) in (select (v1) from semiUp3238806025272546318);
...
select COALESCE(SUM(annot), 0) as v9 from aggJoin2356568418999642008;
