create or replace view semiUp1445922238344045244 as select MessageId as v1 from Person_likes_Message_T AS Person_likes_Message_T where (MessageId) in (select MessageId from Message_hasTag_Tag_T AS Message_hasTag_Tag_T);
create or replace view semiUp9073287018920832714 as select ParentMessageId as v1 from Comment_replyOf_Message_T AS Comment_replyOf_Message_T where (ParentMessageId) in (select v1 from semiUp1445922238344045244);
create or replace view semiUp7930797179927766388 as select MessageId as v1 from Message_hasCreator_Person_T AS Message_hasCreator_Person_T where (MessageId) in (select v1 from semiUp9073287018920832714);
...
create or replace view semiDown105833099928570853 as select MessageId as v1 from Message_hasTag_Tag_T AS Message_hasTag_Tag_T where (MessageId) in (select v1 from semiDown6383762215250526098);
...
select COALESCE(SUM(annot), 0) as v9 from aggJoin2679650060739810556;
