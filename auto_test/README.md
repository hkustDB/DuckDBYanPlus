# DuckDB v1.5 experiment guide

[Back to the project overview](../README.md)

All commands below are run from the repository root, not from `auto_test/`.
The `batch_run.sh` and `auto_run.sh` benchmark launchers require Linux `taskset`.

## Configuration and supported queries

### Cyclic-query scope

When GYO detects a cyclic core, the experimental virtual-filter path uses the
binary join plan selected by DuckDB's v1.5 dynamic-programming enumerator. Yan+
derives a virtual bag boundary from that plan and treats its existing left and
right subtrees as the probe and build bags for semi-join-filter insertion.

On this path, the virtual bag is optimizer metadata, not a new bag or SCOJ
operator. The selected join topology, join conditions, and residual predicates
remain unchanged. Existing `CREATE_BF` and `USE_BF` wrappers build and apply the
filter around the complete subtrees, after which the original DuckDB join still
executes. Cyclic queries such as LSQB Q2 retain their native aggregate above
this filter-wrapped join tree.

Supported decomposable cyclic `COUNT(*)` queries can instead use exact
annotation-based aggregation pushdown. Eligible cases, such as Graph Q4, can
select a two-cyclic-bag join plan. This path omits `CREATE_BF` and `USE_BF`.
These are specialized experimental paths, not an enumeration of all GHD
decompositions or a complete GHD/SCOJ execution strategy. Unsupported query
shapes retain DuckDB's native optimizer path.
...
The batch defaults to 64 threads and Linux CPU list `0-31,36-67`. Set
`YANPLUS_THREADS` and `YANPLUS_CPU_LIST` to match your machine. Run
`./batch_run.sh --help` for the full option list.
...
`dsb_agg_rewrite` queries and six `dsb_spj_rewrite` queries, including SPJ q99.
There is no JOB suite in this existing baseline.
...
Before timing, the runner checks each rewrite against its `query.sql` result.
Matching rewrites are marked `ok`. If the original errors, times out, or is
omitted, successful rewrites are marked `unverified` and are still timed; they
are not treated as exact-result matches. Rewrite mismatches, SQL errors, and
timeouts are ineligible for timing. Timing runs in globally randomized
repetition blocks, one fresh DuckDB process per plan.

The timestamped result directory contains raw
...
### Batch launcher options and result files
...
ignored by Git. Start from the [anonymous example](../.artifact-local.example.json)
and edit only the ignored copy:
...
| JOB balanced partitions 1–5 | `job_benchmark_figure_1` through `job_benchmark_figure_5` |
| All JOB queries | `job_benchmark_all` |
...
rendered as five family-range figures, five balanced-partition figures, and
one all-query figure. Its query labels are horizontal, and range captions
such as `JOB queries 28-33` are omitted. The centered legend is
